      ...
      <div className="panel">
        ...
        {state.boss && (
          <div style={{ marginTop: "0.75rem" }}>
            ...
            <HpBar hp={state.boss.hp} maxHp={state.boss.maxHp} />
      ...
      {(state.phase === "round" || state.phase === "finished") && (
        <div className="panel">
          ...
          <div className="btn-row" style={{ alignItems: "stretch", flexWrap: "wrap" }}>
            {state.participants.map((p) => (
              <RaidFighterPanel key={p.userId} participant={p} isSelf={p.userId === user.id} mvp={p.userId === state.mvpUserId} />
            ))}
          </div>
        </div>
      )}
      ...
      {f ? (
        <>
          ...
          <div style={{ margin: "0.5rem 0 0.25rem" }}>
            {f.hp <= 0 ? <span style={{ color: "var(--text-dim)" }}>💀 戦闘不能</span> : <FighterVitals hp={f.hp} maxHp={f.maxHp} />}
          </div>
