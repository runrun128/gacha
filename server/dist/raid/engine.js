...
exports.buildBossFighter = buildBossFighter;
exports.buildRaidFighter = buildRaidFighter;
exports.aliveParticipantIds = aliveParticipantIds;
exports.actionableParticipantIds = actionableParticipantIds;
exports.applyRoundStartEffects = applyRoundStartEffects;
exports.resolveParticipantActions = resolveParticipantActions;
exports.bossTurn = bossTurn;
const game_core_1 = require("@identity-slot/game-core");
const engine_1 = require("../battle/engine");
function buildBossFighter(bossKey, participantCount) {
    const info = game_core_1.BOSSES[bossKey];
...
function buildRaidFighter(character, userId, displayName) {
    const base = (0, engine_1.buildFighter)(character, userId, displayName);
...
function aliveParticipantIds(room) {
    return room.participantIds.filter((pid) => (room.fighters[pid]?.hp ?? 0) > 0);
}
function actionableParticipantIds(room) {
...
function applyRoundStartEffects(room) {
    ...
    for (const pid of room.participantIds) {
        ...
        if (f.atkDebuffRounds > 0)
            f.atkDebuffRounds -= 1;
        if (f.silencedRounds > 0)
            f.silencedRounds -= 1;
    }
    const boss = room.boss;
    if (boss.shieldRounds > 0)
        boss.shieldRounds -= 1;
    if (boss.atkDebuffRounds > 0)
        boss.atkDebuffRounds -= 1;
    if (boss.poisonRounds > 0 && boss.hp > 0) {
        const dmg = Math.round(boss.maxHp * game_core_1.BURN_DAMAGE_PERCENT);
        boss.hp -= dmg;
        boss.poisonRounds -= 1;
        room.log.push(`🕷️ ${boss.name} は毒で ${dmg} ダメージ!`);
    }
}
function resolveGambleVsBoss(actor, boss) {
    actor.gambleCooldown = game_core_1.GAMBLE_COOLDOWN_ROUNDS;
    if (Math.floor(Math.random() * 100) + 1 <= game_core_1.GAMBLE_SUCCESS_CHANCE) {
        boss.hp = 0;
        return [
            `💀 ${actor.displayName} の一か八かの一撃が決まった! ${engine_1.GAMBLE_SUCCESS_QUOTE}`,
            `${boss.name} を一撃で沈めた!!`,
        ];
    }
    const { damage } = (0, engine_1.computeDamage)(actor.atk, boss.def, 0.4, 1.0, actor.luck, false);
    boss.hp -= damage;
    const recoil = Math.round(actor.maxHp * game_core_1.GAMBLE_RECOIL_PERCENT);
    actor.hp = Math.max(0, actor.hp - recoil);
    return [
        `💀 ${actor.displayName} の一か八かの一撃…不発。${(0, engine_1.pick)(engine_1.GAMBLE_FAIL_QUOTES)} ` +
            `${boss.name} に ${damage} ダメージ、反動で自分も ${recoil} ダメージを受けた。`,
    ];
}
function resolveItemEffectVsBoss(itemKey, actor, boss) {
    ...
    switch (item.effect) {
        ...
        case "priority_attack": {
            const { damage, crit } = (0, engine_1.computeDamage)(actor.atk, boss.def, item.value, 1.0, actor.luck);
            boss.hp -= damage;
        ...
        case "enemy_atk_down": {
            boss.atkDebuffRounds = Math.max(boss.atkDebuffRounds, Math.trunc(item.value) || game_core_1.ATK_DEBUFF_ROUNDS);
        ...
        case "extra_turn": {
            const { damage, crit } = (0, engine_1.computeDamage)(actor.atk, boss.def, 1.0, 1.0, actor.luck);
            boss.hp -= damage;
            return [
...
function resolveParticipantActions(room, weakPointActive) {
    const boss = room.boss;
    const acting = actionableParticipantIds(room);
    for (const pid of acting) {
        if (!room.pending[pid])
            room.pending[pid] = { type: "attack" };
    }
    ...
    const featherIds = acting.filter((pid) => {
        ...
        return act?.type === "item" && act.itemKey && game_core_1.ITEMS[act.itemKey]?.effect === "priority_attack";
    });
    ...
    for (const pid of order) {
        ...
        if (action.type === "defend") {
            const heal = Math.round(f.maxHp * game_core_1.DEFEND_HEAL_PERCENT);
            f.hp = Math.min(f.maxHp, f.hp + heal);
            room.log.push(`🛡️ ${name} は防御の構え。${(0, engine_1.pick)(engine_1.DEFEND_QUOTES)}`);
        ...
        let atkValue = f.atk;
        if (f.atkDebuffRounds > 0)
            atkValue = Math.round(f.atk * game_core_1.ATK_DEBUFF_MULT);
        let multiplier;
        if (action.type === "special") {
            const stype = f.specialType ?? "attack";
            f.specialCooldown = game_core_1.SPECIAL_COOLDOWN_ROUNDS;
            room.log.push(`${game_core_1.SPECIAL_TYPES[stype].emoji} ${name} のとくぎ「${f.moveName}」(${game_core_1.SPECIAL_TYPES[stype].label})! ${(0, engine_1.pick)(engine_1.SPECIAL_QUOTES)}`);
            if (stype === "heal") {
                const lines = [];
                for (const allyId of room.participantIds) {
                    ...
                    if (!ally || ally.hp <= 0)
                        continue;
                    const heal = Math.round(ally.maxHp * game_core_1.HEAL_SPECIAL_PERCENT_RAID);
            ...
            if (stype === "shield") {
                f.invincibleRounds = Math.max(f.invincibleRounds, game_core_1.SPECIAL_SHIELD_ROUNDS);
                room.log.push(`　→ ${game_core_1.SPECIAL_SHIELD_ROUNDS}ラウンドの間、被ダメージを無効化する盾を展開した!`);
                continue;
            }
            if (stype === "debuff") {
                boss.atkDebuffRounds = Math.max(boss.atkDebuffRounds, game_core_1.ATK_DEBUFF_ROUNDS);
                room.log.push(`　→ ${boss.name} の攻撃力を${game_core_1.ATK_DEBUFF_ROUNDS}ラウンドの間下げた!`);
                continue;
            }
            multiplier = game_core_1.SPECIAL_ATTACK_MULTIPLIER;
        }
        else {
            multiplier = 1.0;
            room.log.push(`⚔️ ${name} のこうげき! ${(0, engine_1.pick)(engine_1.ATTACK_QUOTES)}`);
        }
        let weakNote = "";
        if (weakPointActive) {
            multiplier *= game_core_1.WEAKPOINT_MULTIPLIER;
            weakNote = "🌟隙を突いた! ";
        }
        let shieldNote = "";
        if (boss.shieldRounds > 0) {
            multiplier *= game_core_1.BOSS_SHIELD_REDUCTION;
            shieldNote = "🌪️(盾で軽減) ";
        }
        const { damage, crit } = (0, engine_1.computeDamage)(atkValue, boss.def, multiplier, 1.0, f.luck);
        boss.hp -= damage;
    ...
    return { bossDefeated: boss.hp <= 0 };
}
/** ボスの行動。boss_turn() 相当。 */
function bossTurn(room) {
    const boss = room.boss;
    ...
    if (boss.atkDebuffRounds > 0)
        bossAtkValue = Math.round(boss.atk * game_core_1.ATK_DEBUFF_MULT);
    if (!boss.healedOnce && boss.hp <= boss.maxHp * 0.5 && Math.floor(Math.random() * 100) + 1 <= game_core_1.BOSS_SELFHEAL_CHANCE) {
        boss.healedOnce = true;
        const heal = Math.round(boss.maxHp * game_core_1.BOSS_SELFHEAL_PERCENT);
    ...
    if (room.bossKey === "varga" && !boss.roarUsed && !boss.enraged && boss.hp <= boss.maxHp * (info.roarThreshold ?? 0.6)) {
        ...
        for (const pid of aliveIds)
            room.fighters[pid].atkDebuffRounds = game_core_1.ATK_DEBUFF_ROUNDS;
        room.log.push(`　→ 参加者全員の攻撃力が${game_core_1.ATK_DEBUFF_ROUNDS}ラウンドの間低下した!`);
        return;
    }
    ...
    if (room.bossKey === "gehenna" && room.roundNo % 3 === 0) {
        ...
        for (const pid of aliveIds) {
            ...
            const defMult = (0, engine_1.defenseMultiplier)(target, room.pending[pid]);
            const { damage, crit } = (0, engine_1.computeDamage)(bossAtkValue, target.def, aoeMultiplier, defMult, 0);
            target.hp -= damage;
    ...
    if (room.bossKey === "gehenna" && Math.floor(Math.random() * 100) + 1 <= (info.burnMoveChance ?? 0)) {
        ...
        const defMult = (0, engine_1.defenseMultiplier)(target, room.pending[targetId]);
        const { damage, crit } = (0, engine_1.computeDamage)(bossAtkValue, target.def, 1.1, defMult, 0);
        target.hp -= damage;
        target.burnRounds = game_core_1.BURN_ROUNDS;
        room.log.push(`　→ ${crit ? "会心の一撃! " : ""}${target.displayName} に ${damage} ダメージ!さらに${game_core_1.BURN_ROUNDS}ラウンドの間、火傷状態になった。`);
        return;
    }
    if (room.bossKey === "lilith" && room.roundNo % 3 === 0) {
        ...
        const target = room.fighters[targetId];
        const defMult = (0, engine_1.defenseMultiplier)(target, room.pending[targetId]);
        const { damage, crit } = (0, engine_1.computeDamage)(bossAtkValue, target.def, 1.5, defMult, 0);
        target.hp -= damage;
    ...
    if (room.bossKey === "lilith" && aliveIds.length >= 2 && Math.floor(Math.random() * 100) + 1 <= (info.blizzardMoveChance ?? 0)) {
        ...
        for (const targetId of targets) {
            const target = room.fighters[targetId];
            const defMult = (0, engine_1.defenseMultiplier)(target, room.pending[targetId]);
            const { damage, crit } = (0, engine_1.computeDamage)(bossAtkValue, target.def, 0.8, defMult, 0);
            target.hp -= damage;
            room.log.push(`　→ ${target.displayName} に ${crit ? "会心! " : ""}${damage} ダメージ!`);
        }
        return;
    ...
    if (room.bossKey === "voltex" && !boss.shieldUsed && boss.hp <= boss.maxHp * (info.shieldThreshold ?? 0.4)) {
        ...
        boss.shieldRounds = game_core_1.BOSS_SHIELD_ROUNDS;
        room.log.push(`🌪️ ${boss.name} が「${info.shieldMoveName}」を展開! ${info.shieldMoveQuote}`);
        room.log.push(`　→ ${game_core_1.BOSS_SHIELD_ROUNDS}ラウンドの間、受けるダメージが軽減される!`);
        return;
    }
    if (room.bossKey === "voltex" && room.roundNo % 3 === 0) {
        ...
        for (const targetId of targets) {
            const target = room.fighters[targetId];
            const defMult = (0, engine_1.defenseMultiplier)(target, room.pending[targetId]);
            const { damage, crit } = (0, engine_1.computeDamage)(bossAtkValue, target.def, 1.0, defMult, 0);
            target.hp -= damage;
            room.log.push(`　→ ${target.displayName} に ${crit ? "会心! " : ""}${damage} ダメージ!`);
        }
        return;
    ...
    if (room.bossKey === "arachne" && room.roundNo % 3 === 0) {
        ...
        const targetId = pickOne(aliveIds);
        const target = room.fighters[targetId];
        target.silencedRounds = game_core_1.SILENCE_ROUNDS;
        room.log.push(`　→ ${target.displayName} の力が封じられた!${game_core_1.SILENCE_ROUNDS}ラウンドの間、とくぎ・一か八かが使えない。`);
        return;
    }
    if (room.bossKey === "shade" && room.roundNo % 3 === 0) {
        ...
        const target = room.fighters[targetId];
        const defMult = (0, engine_1.defenseMultiplier)(target, room.pending[targetId]);
        const { damage, crit } = (0, engine_1.computeDamage)(bossAtkValue, target.def, 1.4, defMult, 0);
        target.hp -= damage;
        const drain = Math.round(damage * (info.drainRatio ?? 0.5));
        boss.hp = Math.min(boss.maxHp, boss.hp + drain);
        room.log.push(`　→ ${crit ? "会心の一撃! " : ""}${target.displayName} に ${damage} ダメージ!${boss.name} はそのうち ${drain} を吸い取り自らのHPに変えた。`);
        return;
    }
    const quote = (0, engine_1.pick)(info.attackQuotes);
    room.log.push(`${boss.name} ${quote}`);
    const targetId = pickOne(aliveIds);
    const target = room.fighters[targetId];
    const defMult = (0, engine_1.defenseMultiplier)(target, room.pending[targetId]);
    const { damage, crit } = (0, engine_1.computeDamage)(bossAtkValue, target.def, enrageMultiplier, defMult, 0);
    target.hp -= damage;
    ...
    if (room.bossKey === "arachne" && Math.floor(Math.random() * 100) + 1 <= (info.poisonChance ?? 0)) {
        target.poisonRounds = game_core_1.POISON_ROUNDS;
        room.log.push(`🕷️ ${target.displayName} は猛毒に侵された!${game_core_1.POISON_ROUNDS}ラウンドの間、毒状態になる。`);
    }
    if (room.bossKey === "shade" && Math.floor(Math.random() * 100) + 1 <= (info.curseMoveChance ?? 0)) {
        ...
        room.log.push(`🌑 ${info.curseMoveQuote} ${target.displayName} の攻撃力が${game_core_1.ATK_DEBUFF_ROUNDS}ラウンドの間下がった!`);
    }
}
