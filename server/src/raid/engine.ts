...
import { RaidBoss, RaidFighter, RaidRoom } from "./types";
...
export function resolveParticipantActions(room: RaidRoom, weakPointActive: boolean): { bossDefeated: boolean } {
  const boss = room.boss!;
  ...
  for (const pid of order) {
    ...
    if (action.type === "item" && action.itemKey) {
      ...
      if (boss.hp <= 0) return { bossDefeated: true };
      continue;
    }
...
/** ボスの行動。boss_turn() 相当。 */
export function bossTurn(room: RaidRoom) {
  const boss = room.boss!;
